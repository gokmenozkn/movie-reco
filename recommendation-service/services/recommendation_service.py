...
class RecommendationService:
	...
	async def _load_data(self, session: AsyncSession):
		...
		try:
			...
			result = await session.execute(select(Rating.user_id, Rating.movie_id, Rating.rating))
			...
			logging.info(f"Ratings verisi başarıyla yüklendi. Satır sayısı {len(self.ratings_df)}")

		except Exception as e:
			logging.error(f"Veritabanından veri yüklenirken beklenmedik bir hata oluştu: {e}")
	...
	async def get_popular_movies(self, session: AsyncSession, top_n: int = 10, min_ratings_threshold: int = 100):
		"""
	...
	async def get_personalized_recommendations(self, user_id: int, n=10, session: AsyncSession = None):
		pass
